...
public class TimeServerHandler extends ChannelInboundHandlerAdapter {
    ...
    // (1) As explained, the channelActive() method will be invoked when a connection is established and ready to
    ...
    public void channelActive(final ChannelHandlerContext ctx) {
        ...
        // ByteBuf does not have such a method because it has two pointers; one for read operations and the other for
        // write operations. The writer index increases when you write something to a ByteBuf while the reader index
        // does not change. The reader index and the writer index represents where the message starts and ends respectively.
        //
        ...
        // different operation types. You will find it makes your life much easier as you get used to it
        // -- a life without flipping out!
        //
        // Another point to note is that the ChannelHandlerContext.write() (and writeAndFlush()) method returns a
        // ChannelFuture. A ChannelFuture represents an I/O operation which has not yet occurred. It means, any
        // requested operation might not have been performed yet because all operations are asynchronous in Netty.
        // For example, the following code might close the connection even before a message is sent:
        //      Channel ch = ...;
        //      ch.writeAndFlush(message);
        //      ch.close();
        // Therefore, you need to call the close() method after the ChannelFuture is complete, which was returned by
        // the write() method, and it notifies its listeners when the write operation has been done. Please note that,
        // close() also might not close the connection immediately, and it returns a ChannelFuture.
        final ChannelFuture f = ctx.writeAndFlush(time);

        // (4)How do we get notified when a write request is finished then? This is as simple as adding a
        ...
        //$ rdate -o <port> -p <host> --where <port> is the port number you specified in the main() method and <host> is usually localhost.
    }
